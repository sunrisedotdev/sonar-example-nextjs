import { EntityDetails, EntitySetupState, EntityType, SaleEligibility } from "@echoxyz/sonar-core";
        ...
        return (
            <div className={`bg-amber-50 p-2 rounded-md w-full`}>
                <p className={`text-amber-500 w-full`}>
                    No entity found. You must have an entity in Sonar with a linked wallet matching the currently
                    connected wallet.
                </p>
            </div>
        );
...
function EntityDetailRows({ entity }: { entity?: EntityDetails }) {
    ...
    return (
        <div className="flex flex-col gap-1 bg-gray-200 p-2 rounded-md w-full">
            {entity.Label && <span className="text-gray-900">Investing as: {entity.Label}</span>}
...
export default function SonarEntity({ value: entity }: { value?: EntityDetails }) {
    const onClick = () => {
        if (!entity) {
            return;
        }
    ...
    return (
        <div className="flex flex-col gap-2 bg-gray-100 p-4 rounded-xl w-full items-center">
            <h1 className="text-lg font-bold text-gray-900 w-full">Sonar entity details</h1>

            <EntityDetailRows entity={entity} />
            <EntityStateDescription entity={entity} />

            <button className="cursor-pointer bg-gray-900 rounded-xl px-4 py-2 w-fit" onClick={onClick}>
